...
class user
{
    ...
    {
		$sth = $this->connect->prepare("SELECT * FROM ar_account WHERE username = ?");
	...
	{
		$sth = $this->connect->prepare("SELECT * FROM ar_account WHERE facebook_id = ?");
	...
	private function _setLogin($userData)
	{
		...
		try {
			...
			$sth = $this->connect->prepare("UPDATE ar_account SET ip = ? WHERE user_id = ?");
			...
			setcookie('XRLogin', serialize($forCookie), time()+(3600*24), '/');
    ...
    private function _checkLogin()
    {
        ...
        {
			$sth = $this->connect->query("SELECT * FROM ar_account WHERE user_id = " . $_SESSION['user_id']);
			$result = $sth->fetch();
		...
		else if (isset($_COOKIE['XRLogin']) && $_COOKIE['XRLogin'])
		{
			...
			$sth = $this->connect->query("SELECT * FROM ar_account WHERE user_id = " . $cookieData['user_id']);
			$result = $sth->fetch();
		...
		{
					$sth = $this->connect->prepare("UPDATE ar_account SET last_activity = ? WHERE user_id = ?");
