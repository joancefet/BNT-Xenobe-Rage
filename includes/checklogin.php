	...
	else
	{
		...
		if(($user_cookie_ip==$user_ip_address) && ($user_cookie_host==$user_host) && ($user_cookie_agent==$user_agent))
		{
			if ($playerinfo)
			{
				...
				// before updating the players last activity, check to see if its a silent login first. set to 1 hour
				if($timestamp['now'] >= ($timestamp['last'] +3600))
				{
					/*Silent login/reconnect detected*/
					$update = $db->Execute("UPDATE {$db->prefix}account SET last_activity = ? WHERE user_id = ?;", array($stamp, $playerinfo['ship_id']));
					/*add a notification into the users logs*/
					$log_manager = new manage_log();
					$log_manager->player_log($playerinfo['ship_id'],50001,'','','','',"<font color='#6190a5'>Low Priority</font>","<b>Reconnecting Account</b>");
				}
